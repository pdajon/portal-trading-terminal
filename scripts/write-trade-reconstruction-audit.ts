...
async function main() {
  ...
  const outputPath = path.resolve(
    process.cwd(),
    options.output ?? ".portal/audits/trade-reconstruction-latest.json",
  );

  const result = await runHistoricalBaselineScan(request);
  const debugOutput = buildTradeReconstructionDebugOutput(request, result);
  ...
  console.log(
    [
      ...
      `${debugOutput.reconstructionCounts.reconstructedTrades} trades`,
      `${debugOutput.sessionCounts.sessions} sessions`,
      `${debugOutput.confidenceWarnings.length} confidence notes`,
    ].join(" · "),
  );
}
